...
public class TestStripedMap {
    ...
    private static void testMap(final OurMap<Integer, String> map) {
        ...
        assert map.get(34).equals("F") && map.containsKey(34);
        map.forEach((k, v) -> System.out.printf("%10d maps to %s%n", k, v));
        assert map.remove(217).equals("E");
        assert map.size() != 4;
    }
...
class ConcurrentMapTest extends Test {
    ...
    private final int threadCount, nrTrials;
    private final CyclicBarrier startBarrier;
    private final CyclicBarrier stopBarrier;
    private final AtomicInteger keySum;
    private final AtomicIntegerArray countsTotal;

    public ConcurrentMapTest(OurMap<Integer, String> map,
        ...
        this.keySum = new AtomicInteger(0);
        this.countsTotal = new AtomicIntegerArray(threadCount);
        this.startBarrier = new CyclicBarrier(threadCount + 1);
        this.stopBarrier = new CyclicBarrier(threadCount + 1);
    }

    void testSingleThread(int threadNumber) {
        AtomicInteger sum = new AtomicInteger(0);
        map.forEach((k, v) -> {
            if (getThreadFromValue(v) == threadNumber) {
                sum.addAndGet(1);
            }
        });
        try {
            assertEquals(sum.get(), countsTotal.get(threadNumber));
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    private int getThreadFromValue(String value) {
...
//        return Integer.parseInt(value.substring(0, 1));
    ...
    void test() {
        try {
            ...
            startBarrier.await();
            stopBarrier.await();
            assertEquals(map.size(), keySum.get());
    ...
    class Worker implements Runnable {
        ...
        public void run() {
            try {
                ...
                for (int i = 0; i < nrTrials; i++) {
                    int key = random.nextInt(RANDOM_BOUND);
                    String oldValue = map.put(key, makeString(key, threadNumber));
                    if (oldValue == null) {
                        sum++;
                        counts[threadNumber] += 1;
                    } else {
                        int thread = getThreadFromValue(oldValue);
                        // if a value is replaced
                        // e.g. entry (3, 1:3) is replaced with (3, "0:3")
                        if (thread != threadNumber) {
                            counts[thread] -= 1;
                            counts[threadNumber] += 1;
                        }
                    }
                    key = random.nextInt(RANDOM_BOUND);
                    map.containsKey(key);
                    key = random.nextInt(RANDOM_BOUND);

                    if (map.putIfAbsent(key, makeString(key, threadNumber)) == null) {
                        sum++;
                        counts[threadNumber] += 1;
                    }
                    key = random.nextInt(RANDOM_BOUND);
        ...
        private String makeString(int key, int threadNumber) {
            return threadNumber + ":" + key;
        }
        ...
        for(int stripe=0;stripe<lockCount;stripe++){
            synchronized (locks[stripe]){
                for(int hash=stripe;hash<buckets.length;hash+=lockCount){
                    StripedMap.ItemNode<K,V> node=buckets[hash];
                    while(node!=null){
                        consumer.accept(node.k,node.v);
                        node=node.next;
                    }
...
class StripedWriteMap<K,V> implements OurMap<K,V> {
    ...
    public V remove(K k) {
        ...
        synchronized (locks[stripe]){
            bs=buckets;
            final int hash = h % bs.length;
            ItemNode<K,V> node= ItemNode.delete(bs[hash],k,old);
            if(node!=null){
                bs[hash]=node;
    ...
    public void forEach(Consumer<K,V> consumer) {
        ...
        for(int stripe=0;stripe<lockCount;stripe++){
            for(int hash=stripe;hash<bs.length;hash+=lockCount){
                StripedWriteMap.ItemNode<K,V> node=bs[hash];
                while(node!=null){
                    consumer.accept(node.k,node.v);
                    node=node.next;
                }
