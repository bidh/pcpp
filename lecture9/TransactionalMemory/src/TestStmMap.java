...
class StmMap<K,V> implements OurMap<K,V> {
  ...
  public V put(K k, V v) {
      return atomic(()->{
          ...
          cachedSize.increment( newNode == node ? 1 : 0);
          return old.get();
      });
  }
  ...
  public V putIfAbsent(K k, V v) {
    return atomic(()->{
        ...
        final TxnRef<ItemNode<K,V>>[] bs = buckets.get();
        final int hash = h % bs.length;
        if(ItemNode.search(bs[hash].get(), k, null)){
            return bs[hash].get().v;
        }else{
            bs[hash].set(new ItemNode<K,V>(k, v, bs[hash].get()));
  ...
  public V remove(K k) {
      return atomic(()->{
          ...
          final TxnRef<ItemNode<K,V>>[] bs = buckets.get();
          final int hash = h % bs.length;
          ItemNode<K,V> node= ItemNode.delete(bs[hash].get(),k,old);
          if(node!=null){
              bs[hash].set(node);
  ...
  public void forEach(Consumer<K,V> consumer) {
      final int length=atomic(()->buckets.get().length);
      for(int i=0;i<length;i++){
          int h=i;
          ItemNode<K,V> nodes=atomic(()-> buckets.get()[h].get());
          while(nodes!=null){
