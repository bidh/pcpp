...
public class TestLiftGui {
    ...
    public static void main(String[] args) {
        ...
        List<Runnable> lstRunnable = new ArrayList<>();
        lstRunnable.add(lift1);
        lstRunnable.add(lift2);
        lstRunnable.add(lift3);
        lstRunnable.add(lift4);
        final double steps = 16.0;
        int scheduledTime=(int)(1000/steps);
        for(int i=0;i<lstRunnable.size();i++){
            executor.scheduleAtFixedRate(lstRunnable.get(i), 0,scheduledTime,TimeUnit.MILLISECONDS);
        }
...
class Lift implements Runnable {
    ...
    private synchronized void setStop(int floor, Direction dir) {
        ...
        SwingUtilities.invokeLater(()->this.changeButtonColor(floor-lowFloor,Color.RED));
    }
    ...
    public void run() {
        final double steps = 16.0;
            if(isOpening){
                this.doorRange+=1.0/steps;
                shaft.moveTo(floor,doorRange);
                updateDoorState();
            }else if(isClosing){
                this.doorRange-=1.0/steps;
                shaft.moveTo(floor,doorRange);
    ...
    private void closeDoor(){
        ...
        SwingUtilities.invokeLater(() -> changeButtonColor((int) floor - lowFloor, Color.BLACK));
    }
    private void neutralizeDoor() {
        this.isOpening = false;
        this.isClosing = false;
    }
    private void updateDoorState(){
        if(this.doorRange==0.0){
            if(this.isClosing) neutralizeDoor();
            else openDoor();
        }else if(this.doorRange==1.0 && isOpening){
            closeDoor();
        }
    }
    ...
    public void changeButtonColor(int index, Color color) {
        this.buttons.get(index).setForeground(color);
    }
