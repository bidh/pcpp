...
public class TestFetchWebGui {
    ...
    private static void goodFetch() {
        ...
        for(int i=0;i< urls.length;i++){
            downloadTask[i]=new DownloadWorker(textArea,i);
        }
       ...
       for(int i=0;i<urls.length;i++){
            downloadTask[i].addPropertyChangeListener(new PropertyChangeListener() {
    ...
    static class DownloadWorker extends SwingWorker<String,String> {
        ...
        private final int i;
        public DownloadWorker(TextArea textArea,int i) {
        ...
        public String doInBackground() {
            ...
            String page = getPage(urls[i], 200),
                    result = String.format("%-40s%7d%n", urls[i], page.length());
            count.getAndIncrement();
            setProgress((100*count.get())/urls.length); // (2)
            publish(result); // (4)
            return result;
            //return sb.toString();
        }
